...
**Version:** PAI 5.0.0 | Algorithm v6.4.15 | Memory v7.6
...
- **Version:** v6.4.15 (canonical: read `PAI/ALGORITHM/LATEST`)
- **Location:** `PAI/ALGORITHM/` (canonical pointer: `LATEST` → `v{VERSION}.md`)
- **CLI:** `bun PAI/TOOLS/algorithm.ts`
- **Full doc:** `PAI/DOCUMENTATION/Algorithm/AlgorithmSystem.md`
- **Doctrine highlights (current through v6.4.15):** Task-class adversarial catalog (v6.4.15, OBSERVE/THINK) — the v6.4.5 seed-auditor reflex gains a lookup of recurring hazard *shapes* beyond the v6.4.12 bulk-commit class: a **file-store fire/dispatch executor** seeds ISCs for double-fire (claim-inside-lock-before-dispatch) / fire-after-cancel / poison-entry / corrupt-line data-loss / lost-update; a **path/glob matcher** for case-fold / win32-separator / trailing-slash / junction≠symlink; a **file-diff/patch/stream tool** for trailing-newline / CRLF↔LF / empty-file / SIGPIPE-on-a->64KB-diff; bulk-commit and multi-tree shapes are cross-referenced (v6.4.12 / Gate H v6.4.14), not re-seeded; adds a trigger, not a new gate; Cross-tree + cross-platform blast-radius (v6.4.14) — Gate H's emitter census gains an **out-of-tree sweep** (after moving a top-level dir or OS-referenced path constant, census Windows scheduled tasks, Startup-folder `.vbs`/`.lnk`, `.bat`/`.ps1` wrappers, `launchd`, cron — they fail silently, so an in-tree grep is a false all-clear) and a **cross-tree symmetry** clause (before editing across live/fork/install trees, diff the paths for copy-vs-divergent and enumerate the writer/reader counterpart in each tree); plus a **Platform-Shape Preflight** at THINK — enumerate OS-terminal failure shapes up front (win32 dir-rename EPERM→robocopy, `HOME ?? USERPROFILE ?? homedir()`, junction≠symlink, CRLF-before-first-edit, no `.sh`/`launchctl` on Windows) rather than discovering them reactively; Cato dud-verdict auto-fallback to Forge (v6.4.13, VERIFY) — when the Cato cross-vendor audit returns skipped/error/no-parseable-verdict, the Rule 2a table auto-spawns Agent(subagent_type="Forge") ONCE as the cross-family fallback auditor (no user prompt), escalating to the user if Forge also duds (single-shot, no recursion); applies to whatever the Cato agent surfaces, not just the tool's stdout; closes the recurring manual-switch friction after a dud Cato run; Consumer-contract probe (v6.4.13, THINK) — before designing a producer (a field, fix, assertion, payload another surface dereferences), read the REAL consumer/source that will dereference it, not a description/docstring/replica/proxy; distinct from the Installed-runtime version probe and the design-time complement to VERIFY Rule 1b (1b proves the probe hit the real target at verify-time, this ensures you built against it at design-time); Bulk/generated-commit seed-auditor class (v6.4.12, OBSERVE/THINK) — a >20-file or regenerated/generated commit auto-seeds adversarial ISCs for a multi-hit secret scan (the scanner short-circuits on the first hit, so "fixed the flagged file" is a false green on a large commit), a deletion census over every dropped/renamed path, an EOL/binary/encoding check, and a generated-content-survives-regen guard test (a snapshot-only edit is self-deleting); extends the v6.4.5 seed-auditor rule from code operations to commit shapes; Target Authenticity (Verify Rule 1b, v6.4.11, VERIFY) — a green counts only if its probe hit the REAL production path: every `[x]` on a user-facing/behavioral ISC names the exact path exercised and asserts it is not an isolated submodule, not the importee when importers are the contract, not a self-referential test, and not the live environment where a scratch fixture was intended (env/path-sensitive probes use a discriminator distinct from the live value); a proxy green is a false green and the ISC stays open; Rule 1 proves a probe ran, 1b proves it ran against the thing; Scanner-First Ground Truth (v6.4.10, OBSERVE/BUILD) — when a deterministic tool (test suite, full-reference `grep`, `SymbolCensus.ts`, linter, secret-scan) can settle a claim, run it FIRST as ground truth before reading files/doc-labels to confirm or making the first edit, and enumerate the whole hit-class up front; the OBSERVE-phase upstream cousin of the Contract-Capture Reflex, plus a Gate F input-artifact staleness clause (fingerprint inputs the run didn't produce before aggregating); Delegation Digest Gate (v6.4.9, EXECUTE/VERIFY) — consume a spawned agent's output lean: take a structured digest (verdict + findings + diff/artifact path) not the raw transcript, review the diff not the tree, batch related asks, and let the agent write its own files; reading a worker's full transcript collapses the conductor's prompt cache (consumption discipline, not a delegation floor); Gate F count-provenance (v6.4.8, OBSERVE) — a count is a derived fact, not an observation: before quoting any aggregate (pass/fail tally, item count, benchmark score, percentage, mean) success-filter the underlying runs, record each contributing run's source, and state the denominator; a count quoted before the filter is a conjecture; Contract-Capture Reflex (v6.4.7, BUILD) — before the first edit on a change carrying a behavioral or positional contract, capture it first: run/read the covering test (or write a characterization test) for code, and `grep` for positional back-references (`Rule N`/`ISC-N`/`Gate X`/section ordinals) before renumbering doctrine; the cheap general-edit complement to the Security-Surface proof-of-fire; Loop & Batch Control (v6.4.6) — iterate-until-good loops declare a termination contract (max-iters + no-progress-after-2 + budget cap) at PLAN, and recoverable mid-run events self-continue with N-loss accounting rather than halt a viable batch; Preflight Gate H — Deletion-Risk Census (`SymbolCensus.ts`, run at OBSERVE recon; covers symbol deletes, shared-model loosening, and constant/path moves; `--symbols` batch for whole-module prunes); seed auditor adversarial cases as ISC sub-criteria at OBSERVE (v6.4.5); Gate G — Config-as-Behavior/Blast-Radius (v6.4.2); Gate F — Provenance-at-OBSERVE (v6.4.1); Rule 2a cross-vendor audit (Cato) MANDATORY at E4/E5 and correctness-critical E3 (v6.4.2); Security-Surface Edit Gate (HARD, BUILD) — proof-of-fire precedes editing `hooks/security/**`/`command-normalize.ts`/`PATTERNS.yaml`, delegated at E3+, never self-certified; Preflight Gate E (OBSERVE) — external report/changelog/audit claims are conjectures, live-probe before scoping; installed-runtime probe + convention-files first-batch read (v6.4.3); auto-extract shared helper on second consumer; (v6.3.0) twelve-section ISA in fixed order; closed enumeration of thinking capabilities (IterativeDepth, ApertureOscillation, FeedbackMemoryConsult, Advisor, ReReadCheck, FirstPrinciples, SystemsThinking, RootCauseAnalysis, Council, RedTeam, Science, BeCreative, Ideate, BitterPillEngineering, Evals, WorldThreatModel, Fabric patterns, ContextSearch, ISA); Capability-Name Audit Gate (phantom names = CRITICAL FAILURE); ID-stability rule (ISC IDs never re-number on edit)
...
| **Algorithm** | `Algorithm/LATEST` → `Algorithm/v{VERSION}.md` (currently v6.4.15), `Algorithm/capabilities.md`, `Algorithm/mode-detection.md`, `hooks/ISASync.hook.ts` → `MEMORY/WORK/{slug}/ISA.md`, `skills/ISA/` (canonical Scaffold/Append/Reconcile workflows) |
