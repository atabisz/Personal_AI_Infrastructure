...
**Forge as Cato-fallback auditor (NEW v6.4.13):** Forge also serves as the automatic cross-vendor AUDITOR when Cato (`CrossVendorAudit.ts`) returns a dud verdict — `skipped` / `error` / empty-unparseable (codex flake, timeout, Windows capture miss). Per Rule 2a, the executor auto-spawns `Agent(subagent_type="Forge")` to run the audit in Cato's place with no user prompt, instructing Forge to emit Cato's verdict shape so downstream Rule 2a handling is unchanged. Terminal case: if codex is entirely absent, Forge cannot run either — escalate to the user rather than loop or proceed same-family. This closes the recurring manual-switch friction (reflection L127) that persisted after the Cato `-o FILE` capture fix.
...
- **Forge (GPT-5.5, codex exec):** localized completion speed, quality/completeness focus. Default producer at E3/E4/E5. Pick when the change is bounded to a small surface and the verification bar is "every branch is real."
...
| SymbolCensus (deletion-risk census) | **Before deleting/pruning/moving/renaming a code symbol or module on the premise "nothing uses this," or loosening a shared model (Preflight Gate H, v6.4.3; OBSERVE-timing + batch + shared-model v6.4.4).** Read-only TS/JS tool: classifies every occurrence of a symbol by KIND (declaration / import-export / type-position / value-position / test-file / comment-string) and returns a fail-safe verdict. `--symbols a,b,c` runs a batch census emitting a keep/cut table for a whole-module prune. Run at OBSERVE recon, not the mutating step. Defeats the grep-count-is-not-usage trap — a symbol is live through union-type membership, internal references, or test-only usage a count misses. **v6.4.14: out-of-tree emitter sweep** (a top-level dir / OS-referenced path move censuses scheduled tasks, Startup-folder launchers, `.bat`/`.ps1` wrappers, `launchd`, cron — they fail silently, so an in-tree grep is a false all-clear) and **cross-tree symmetry** (before editing across live/fork/install trees, diff the paths for copy-vs-divergent and enumerate the writer/reader counterpart in each tree). NOT a thinking capability; selected like ReferenceCheck. | `bun ~/.claude/PAI/TOOLS/SymbolCensus.ts <symbol> [root] [--json]` · batch: `--symbols a,b,c [root]` |
