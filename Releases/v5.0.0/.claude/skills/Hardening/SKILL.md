...
| **MutationTest** | Stub | Stryker integration; deferred |
...
- **fast-check** — the property-testing primitive used throughout PropertyTest (ISCs as universally-quantified claims, deterministic shrinking to minimal counterexamples).
- **ISA format** — property-based criteria land as a `bun-property` ISC convention (property / generator / runs) in the ISA's `## Test Strategy` (see `PAI/DOCUMENTATION/IsaFormat.md`). This is a Hardening-introduced criterion type; keep the shape consistent with the existing `bun-test` rows.
- **Algorithm** — a VERIFY-phase Hardening gate is a natural extension point at E4/E5 (`PropertyAudit` as a candidate capability). Not yet part of the live Algorithm doctrine — treat as advisory until adopted.
- **System prompt** — verification-as-the-mechanism framing (`PAI/PAI_SYSTEM_PROMPT.md`).
