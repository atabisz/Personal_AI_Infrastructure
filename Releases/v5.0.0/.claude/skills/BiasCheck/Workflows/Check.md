...
2. **Is it the right tool?** Pure opinion piece with no empirical scaffolding → redirect to `ContentAnalysis`. Pure claim about a person's character → redirect to `PrivateInvestigator` or `Research`. A study, dataset, survey, statistical claim, or article reporting any of those → proceed.
...
Be specific. If a consultancy sells AI workforce consulting, their conclusion that companies need AI workforce consulting is structurally compromised — but the data may still be useful. Note the multiplier on the analysis, don't reflexively disqualify.
