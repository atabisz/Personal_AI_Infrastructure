...
`~/.claude/PAI/USER/SKILLCUSTOMIZATIONS/BiasCheck/`
...
```
User: "bias check https://example.com/some-article-citing-a-vendor-survey"
→ Invokes Check workflow
→ Fetches the article, identifies the cited vendor study, fetches the vendor's source
...
- **Vendor research is research; treat it that way.** Don't dismiss a vendor consulting report because they sell consulting. Their funding bias is a multiplier on the analysis, not an automatic disqualifier. Specific finding > generic "vendor bias."
...
- **Don't go beyond the source you fetched.** No drive-by claims about a firm's other reports if you only read one. No "they always do X" — stick to what's in the artifact.
- **Pure opinion piece with no data?** This skill is the wrong tool — redirect to `ContentAnalysis` for author/rhetorical read. BiasCheck needs claims with empirical scaffolding.
- **Author background is out of scope.** This skill audits the source artifact, not the author's biography. If you need "who is this writer and what's their track record," use Research or PrivateInvestigator.

## Execution Log

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"BiasCheck","workflow":"Check","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> ~/.claude/PAI/MEMORY/SKILLS/execution.jsonl
```
