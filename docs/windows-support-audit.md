...
| `ContextReduction.hook.sh` is a live registered hook | grep live `settings.json`; glob `hooks/*.sh` | Not registered in live `settings.json`; **no `.sh` files in live `hooks/`**. It appears only in the **release** `settings.json:95`. The statusline `.sh` (`PAI/statusline-command.sh`) exists on disk and is registered in the **release** (`settings.json:370`), but the **live** statusline is an inline bash command, not that file | ⚠️ **RELEASE-ONLY** — not a live concern |
